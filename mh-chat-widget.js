...
btn.onclick = () => {
  ...
  if (eksternChatLoaded && window.zissonWebChat) {
    if (inputDefaults) window.zissonWebChat.setDefaults(inputDefaults);
    window.zissonWebChat.openWidget?.();
    window.startConversation?.();
    return;
  }

  // Første gang: last script + vent til API er klar
  ...
  // Vent på leverandørens event (best practice ifølge dokumentasjonen din)
  window.addEventListener("zissonWebChat", onReady, { once: true });
...
function startEksternChat(inputDefaults) {
  ...
  const script = document.createElement("script");
  script.type = "text/javascript";
  sscript.setAttribute("data-jwt", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..tzOYuMcrK25rEAovnNYsK6kLLUh_Gai6qDvw9dCBfjs");
  script.src = "https://chat2.zisson.com/bootstrapper.js";
  document.body.appendChild(script);
}
  ...
  panel.onsubmit = function (e) {
    ...
    if (window.zissonWebChat.setDefaults) {
      window.zissonWebChat.setDefaults(inputDefaults);
    }
    else {
      window.addEventListener("zissonWebChatLoaded", function () {
        window.zisssonWebchat.setDefaults(inputDefaults);
      });
    }
